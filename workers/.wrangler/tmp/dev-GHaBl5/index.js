  ...
  async fetch(request, env) {
    ...
    if (url.pathname === "/chat" && request.method === "POST") {
      try {
        ...
        const userMessage = String(body?.message ?? "").slice(0, 4e3);
        const sessionId = request.headers.get("x-session-id") || "anon";
        const key = `sess:${sessionId}`;
        const priorJson = await env.SESSIONS.get(key);
        const history = priorJson ? JSON.parse(priorJson) : [{ role: "system", content: "You are a concise, helpful assistant." }];
...
// .wrangler/tmp/bundle-na4qGu/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-na4qGu/middleware-loader.entry.ts
